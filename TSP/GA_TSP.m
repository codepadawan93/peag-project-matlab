...
%
%% Exemple de apel
%[sol,val] = GA_TSP('distante3.txt',200,350,0.8,0.1,1e-4,100);
%% Implementare
...
nrg=0;
OK=1;
% OK=1 daca exista macar 2 valori diferite ale functiei fitness in populatie
while it<=MAX && nrg<=NrG &&OK
    % selectie parinti
     Parinti=selectie_SUS(Pop,dim,n);
     % recombinare
      O=crossover_pop(Parinti,dim,n,pc,c);
      %mutatie
       MO = mutatie_pop(O,dim,n,c,pm);
