function [sol,val] = GA_Regine(n,dim,MAX,pc,pm,ni)
%% Parametrii functiei
%
%% Exemple de apel
% [sol,val] = GA_Regine(10,200,150,0.8,0.1,20);
% [sol,val] = GA_Regine(16,200,250,0.8,0.1,20);
% [sol,val] = GA_Regine(16,400,350,0.8,0.1,40);
%% Implementare
...
while it<=MAX && fitmax<n*(n-1)/2 &&OK
    % selectie parinti
     ...
     % recombinare
      O=crossover_pop(Pop,dim,n,pc);
      %mutatie
       MO = mutatie_pop(O,dim,n,pm);
       %selectia generatiei urmatoare
       GenUrm = Genitor(Pop,MO,dim,n+1,ni);
