function [sol,val] = GA_Knapsack(numec,numev,cmax,dim,MAX,pc,pm,alpha,sigma,eps,NrG)
%% Parametrii functiei
% numec - numele fisierului cost
% numev - numele fisierului valoare

%% Exemplu de apel
%[sol,val] = GA_Knapsack('risc.txt','valoare.txt',1,200,250,0.7,0.05,0.6,0.1,1e-5,80);
%% Implementare
cost=load(numec);
valoare=load(numev);
n=length(cost);
...
while it<=MAX && nrg<=NrG &&OK
    % selectie parinti
     ...
     % recombinare
      O=crossover_pop(Parinti,dim,n,pc,cost,valoare,cmax,alpha);
      %mutatie
       MO = mutatie_pop(O,dim,n,cost,valoare,cmax,pm,sigma);
...
disp(OK);
 ...
 disp(sol);
 disp(['Venitul din dividende:' num2str(val)]);
end
