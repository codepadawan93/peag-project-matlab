...
for i=1:citySize
    for j = i+1:citySize
        %we make it so that there is a 50% chance that two cities don't
        %support eachother
        code = unidrnd(2);
        if code == 1
            confMatrix(i,j) = 1;
...
% keep in mind that the bigger the number of cities, the higher the chances
% of being able to arrange the lords at the table so that no lords in
% conflict are near eachother
%% Implementare
